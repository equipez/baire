...
% Bibliography
\newcommand{\bibfile}{baire.bib}  % Name of the BibTeX file.
...
\usepackage{graphicx}
\usepackage{tikz,tikzscale,pgf,tikz-cd,}
...
Z. Zhang
\thanks{
    School of Mathematics, Sun Yat-sen University, China ({\tt zhangzaikun@mail.sysu.edu.cn}).
}
}
...
\begin{proposition}
  ...
  \begin{enumerate}
    \item For any~$S\subset Y$, we have~$\inter_Y(S) \supset \inter_X(S)$;
      if~$Y$ is open in~$X$, then $\inter_Y(S)=\inter_X(S)$.
    \item For any~$S\subset Y$, we have~$\cl_Y(S) = \cl_X(S) \cap Y$.
  \end{enumerate}
\end{proposition}
...
\begin{corollary}
    ...
    \begin{enumerate}
        \item\label{it:intdense} If each~$S_n$ is open and dense, then~$\bigcap_{n=1}^\infty S_n$ is dense.
        \item\label{it:emptyint} If each~$S_n$ is closed and has empty interior, then~$\bigcup_{n=1}^\infty S_n$ has empty interior.
    \end{enumerate}
\end{corollary}

\begin{proof}
    \ref{it:intdense}.~According to Theorems~\ref{th:intclcm} and~\ref{th:intcllch},
    ...
    Alternatively, we can use item~\ref{it:intdense} and the duality between interior and closure.
\end{proof}
...
\begin{proof}
  ...
  in~$X$ and~$Y\subset \cl_X(S_n)$, item~\ref{it:dense} of Proposition~\ref{prop:dense} ensures
  that~$T_n$ is dense in~$Y$. Hence~$\bigcap_{n=1}^\infty T_n$
  is dense in~$Y$. Thus $Y \subset \cl_X(\bigcap_{n=1}^\infty T_n) \subset
...
\begin{proof}
    ...
    Thus~\eqref{eq:xunion} is impossible as~$X$ is a Baire space.
\end{proof}
...
\begin{proof}
    ...
    (N.B.: The purpose of taking~$Y$, $R$, and~$L$ is to deal with the boundaries~$0$ and~$1$.
...
\begin{proposition}[Uniform boundedness principle]
    \label{prop:ub}

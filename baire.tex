...
\begin{enumerate}
    ...
    \item If~$\bigcup_{n=1}^\infty C_n$ has interior~(for example, it equals~$X$), then at least one~$C_n$ has interior.
\end{enumerate}
...
\begin{proof}
    ...
    \end{align*}
    Note that the restriction of any function in~$R\cap L$ to~$[0,1]$ is nowhere differentiable
    on~$[0,1]$.
...
convergence of~$\{f_k\}$ and the continuity of~$f_0$, for all~$h\ge 0$, we have
    ...
    $C_n$ has no interior, because in any neighborhood of any function~$f\in Y$, there exists
